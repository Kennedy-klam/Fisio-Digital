...
if (isset($_GET["selectdoctor"])) {
    ...
    if (isset($_GET['clientid']) && is_numeric($_GET['clientid'])) {
        ...
        $stmt = $mysqli->prepare("SELECT clientname FROM loginc WHERE clientid = ?");
        ...
        if ($result->num_rows > 0) {
            ...
            $_SESSION["clientname"] = $row['clientname']; // Armazenando o nome do cliente na sessão
        } else {
...
if (!isset($_SESSION["clinicid"])) {
    // Se não estiver definido, você pode obter isso do banco de dados
    $stmt = $mysqli->prepare("SELECT clinicid FROM logind WHERE doctorid = ?");
    $stmt->bind_param('i', $_SESSION["docid"]);
    $stmt->execute();
    $result = $stmt->get_result();

    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $_SESSION["clinicid"] = $row['clinicid']; // Armazenando o clinicid na sessão
    } else {
